import React from "react";

const Productforu = () => {
  const productData = [
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
    {
      name: "พร้อมส่ง PlayStation 5 Console ps 5 ps5 เพลย์ 5 เพลย์สเตชัน 5 บลูเรย์ ดิจิตอล Blu-ray/Disc Digital เครื่องศูนย์ไทย PS5 PS 5",
      imgSrc:
        "https://lzd-img-global.slatic.net/g/p/9959bf748c289ec36823ff3cfadbf7dd.png_200x200q80.jpg_.webp",
      price: 138,
      percentDiscount: 50,
      realPrice: 299,
    },
  ];
  return (
    <div className="productforu">
      <h3>สินค้าเพื่อคุณโดยเฉพาะ</h3>
      <div className="productforu_body">
        {productData.map((e, index) => {
          var shortName = e.name.slice(0, 50);
          return (
            <div
              style={{ background: "#fff" }}
              className="productforu_item"
              key={index}
            >
              <div>
                <img src={e.imgSrc}></img>

                <p style={{ fontSize: "16px", marginRight: "3px" }}>
                  {shortName} ..
                </p>
                <p style={{ fontSize: "16px", color: "#f57224" }}>${e.price}</p>
                <span style={{ display: "flex" }}>
                  <p style={{ fontSize: "13px", color: "#9e9e9e" }}>
                    <strike>${e.realPrice}</strike>
                  </p>
                  <p style={{ color: "#212121", fontSize: "13px" }}>
                    -{e.percentDiscount}%
                  </p>
                </span>
              </div>
            </div>
          );
        })}
      </div>
      <div className="more_product">
        <button>โหลดเพิ่มเติม</button>
      </div>
    </div>
  );
};
export default Productforu;
